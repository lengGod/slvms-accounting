                    ...
                    <table class="table table-bordered text-center">
                        ...
                        <tbody>
                            ...
                            <tr class="table-light">
                                <td colspan="3" class="text-end"><strong>Saldo Awal per
                                        {{ \Carbon\Carbon::parse($startDate)->format('d M Y') }}</strong></td>
                                <td colspan="3"></td>
                                <td colspan="3"></td>
                                <td class="{{ $saldoAwalPokok >= 0 ? '' : 'text-danger' }}">
                                    <strong>{{ number_format($saldoAwalPokok, 0, ',', '.') }}</strong></td>
                                <td class="{{ $saldoAwalBagiHasil >= 0 ? '' : 'text-danger' }}">
                                    <strong>{{ number_format($saldoAwalBagiHasil, 0, ',', '.') }}</strong></td>
                                <td class="{{ $saldoAwalTotal >= 0 ? '' : 'text-danger' }}">
                                    <strong>{{ number_format($saldoAwalTotal, 0, ',', '.') }}</strong></td>
                            </tr>

                            @forelse ($sortedEvents as $event)
                                <tr>
                                    <td>{{ $event['source'] == 'transaction' ? 'TRX' : 'TPN' }}-{{ $event['id'] }}</td>
                                    <td>{{ \Carbon\Carbon::parse($event['date'])->format('d M Y') }}</td>
                                    <td>{{ $event['description'] }}</td>

                                    @php
                                        // Piutang and Titipan Usage are debits to the balance (negative values)
                                        $isDebit = $event['total'] < 0;
                                    ?>

                                    <?php if($isDebit): ?>
                                        <td class="text-danger"><?php echo e(number_format(abs($event['pokok']), 0, ',', '.')); ?></td>
                                        <td class="text-danger"><?php echo e(number_format(abs($event['hasil']), 0, ',', '.')); ?></td>
                                        <td class="text-danger fw-bold"><?php echo e(number_format(abs($event['total']), 0, ',', '.')); ?>
                                    ...
                                    <?php else: ?>
                                        ...
                                        <td></td>
                                        <td></td>
                                        <td class="text-success"><?php echo e(number_format($event['pokok'], 0, ',', '.')); ?></td>
                                        <td class="text-success"><?php echo e(number_format($event['hasil'], 0, ',', '.')); ?></td>
                                        <td class="text-success fw-bold"><?php echo e(number_format($event['total'], 0, ',', '.')); ?>

                                        </td>
                                    <?php endif; ?>

                                    <?php
                                        $saldoPokok += $event['pokok'];
                                        $saldoBagiHasil += $event['hasil'];
                                        $saldoTotal = $saldoPokok + $saldoBagiHasil;
                                    ?>

                                    <td class="<?php echo e($saldoPokok >= 0 ? '' : 'text-danger'); ?>">
                                        <?php echo e(number_format($saldoPokok, 0, ',', '.')); ?></td>
                                    <td class="<?php echo e($saldoBagiHasil >= 0 ? '' : 'text-danger'); ?>">
                                        <?php echo e(number_format($saldoBagiHasil, 0, ',', '.')); ?></td>
                                    <td class="<?php echo e($saldoTotal >= 0 ? '' : 'text-danger'); ?>">
                                        <strong><?php echo e(number_format($saldoTotal, 0, ',', '.')); ?></strong></td>
                                </tr>
                            <?php endforeach; $__env->popLoop(); $loop = $__env->getLastLoop(); if ($__empty_0): ?>
                                <tr>
                                    <td colspan="12" class="text-center">Tidak ada aktivitas pada periode ini.</td>
                                </tr>
                            <?php endif; ?>

                            
                            <tr class="table-active">
                                <td colspan="3" class="text-end"><strong>Saldo Akhir per
                                        <?php echo e(\Carbon\Carbon::parse($endDate)->format('d M Y')); ?></strong></td>
                                <td colspan="3"></td>
                                <td colspan="3"></td>
                                <td class="<?php echo e($saldoPokok >= 0 ? '' : 'text-danger'); ?>">
                                    <strong><?php echo e(number_format($saldoPokok, 0, ',', '.')); ?></strong></td>
                                <td class="<?php echo e($saldoBagiHasil >= 0 ? '' : 'text-danger'); ?>">
                                    <strong><?php echo e(number_format($saldoBagiHasil, 0, ',', '.')); ?></strong></td>
                                <td class="<?php echo e($saldoTotal >= 0 ? '' : 'text-danger'); ?>">
                                    <strong><?php echo e(number_format($saldoTotal, 0, ',', '.')); ?></strong></td>
                            </tr>
                        </tbody>
