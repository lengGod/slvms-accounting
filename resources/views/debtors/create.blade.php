...
@section('content')
    <div class="container-fluid p-4">
        ...
        <div class="card shadow-sm">
            <div class="card-body">
                <form action="{{ route('debtors.store') }}" method="POST" id="debtorForm">
                    @csrf
                    <div class="row g-3">
                        <div class="col-md-6">
                            ...
                            <div class="border rounded p-3 bg-light">
                                <div class="form-check">
                                    <input class="form-check-input balance-type-check" type="checkbox" id="type_pokok"
                                        ...
                                        {{ in_array('pokok', old('balance_type', [])) ? 'checked' : '' }}>
                                ...
                                <div class="form-check mt-2">
                                    <input class="form-check-input balance-type-check" type="checkbox" id="type_bagi_hasil"
                                        ...
                                        {{ in_array('bagi_hasil', old('balance_type', [])) ? 'checked' : '' }}>
                        ...
                        <div class="col-md-6" id="pokokInputContainer" style="display: none;">
                            ...
                            <div class="input-group">
                                ...
                                <input type="number" class="form-control" id="pokok_balance" name="pokok_balance"
                                    value="{{ old('pokok_balance') }}" step="any">
                            </div>
                            <div class="form-text">
                                <i class="bi bi-info-circle me-1"></i>
                                <span id="pokokBalanceHint">Nilai negatif untuk piutang, positif untuk titipan</span>
                            </div>
                            @error('pokok_balance')
                                <div class="text-danger small">{{ $message }}</div>
                            @enderror
                        ...
                        <div class="col-md-6" id="bagiHasilInputContainer" style="display: none;">
                            ...
                            <div class="input-group">
                                ...
                                <input type="number" class="form-control" id="bagi_hasil_balance"
                                    name="bagi_hasil_balance" value="{{ old('bagi_hasil_balance') }}"
                                    step="any">
                            </div>
                            <div class="form-text">
                                <i class="bi bi-info-circle me-1"></i>
                                <span id="bagiHasilBalanceHint">Nilai negatif untuk piutang, positif untuk titipan</span>
                            </div>
                        ...
                        <input type="hidden" id="initial_balance_type" name="initial_balance_type"
                            value="{{ old('initial_balance_type', '') }}">
            ...
            (function() {
                function initDebtorForm() {
                    ...
                    function updatePreview() {
                        ...
                        if (initialBalanceInput) initialBalanceInput.value = totalAmount;
                        if (initialBalanceTypeInput) initialBalanceTypeInput.value = types.join(',');

                        // Update preview
                        if (previewAmount) previewAmount.textContent = formatCurrency(totalAmount);

                        if (!previewType || !previewInfo) return;
                    ...
                    function toggleBalanceInputs() {
                        ...
                        if (pokokInputContainer) pokokInputContainer.style.display = pokokChecked ? 'block' : 'none';
                        if (bagiHasilInputContainer) bagiHasilInputContainer.style.display = bagiHasilChecked ? 'block' : 'none';

                        // Jika tidak ada yang dipilih, sembunyikan error
                        if (!pokokChecked && !bagiHasilChecked && balanceTypeError) {
                    ...
                    if (pokokBalanceInput) pokokBalanceInput.addEventListener('input', updatePreview);
                    if (bagiHasilBalanceInput) bagiHasilBalanceInput.addEventListener('input', updatePreview);
                    ...
                    form.addEventListener('submit', function(e) {
                        ...
                        if (!pokokChecked && !bagiHasilChecked) {
                            e.preventDefault();
                            if (balanceTypeError) balanceTypeError.style.display = 'block';
                ...
                document.addEventListener('DOMContentLoaded', initDebtorForm);
                document.addEventListener('turbo:load', initDebtorForm);
